...
class ChoiceQuestion extends React.Component {
  ...
  showInputField = () => {
    this.setState((prevstate) => ({
      newOptionInput: !prevstate.newOptionInput
    }))
  }
  ...
  render() {
    ...
    return (
      <>
        <AnswerHeader>
          <AnswerLabelCorrect>Correct</AnswerLabelCorrect>
          <AnswerLabelCancel>Cancel</AnswerLabelCancel>
        </AnswerHeader>
        ...
        {options.length > 0 &&
          options.map((option, index) => (
            <OptionChecker key={index}>
              <RadioAnswer>
                <Radio style={{ marginLeft: '10px' }}>{option.name}</Radio>
              </RadioAnswer>
              <AnswerHeader>
                <AnswerLabelCorrect>
